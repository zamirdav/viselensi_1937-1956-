...
sub FOTO_SAVE(TIP,NAM,OPIS)
    ...
    objStream.Type = 1
    objStream.Open
    objStream.LoadFromFile("C:\TEMP\kir-roz\" & NAM)
    FIL_SIZE = 1
    ...
    RS.Fields("Fil_Size").Value = FIL_SIZE
    RS.Fields("TIP_CONT").Value = "mage/jpeg"
...
sub GET_FOTO
    ...
    JPG  = LINE_RED
    X    = LINE_RED
    Response.Write "----- " & TIP & " " & PATH & " <FONT size=1>" & OPIS & "</FONT><BR>" & chr(10)
    if TIP="ФАС" then TIP=1
    if TIP="ПРОФИЛЬ" then TIP=2
    call FOTO_SAVE(TIP,PATH,OPIS)
    X = LINE_RED
end sub
